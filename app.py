...
# ✅ Email format validator helper function
...
# ✅ Fixed: import re removed from here, added email validation
...
def admin_users():
    try:
        ...
        conn.close()
        rows = "".join(
            f"<tr><td>{u['id']}</td><td>{u['name']}</td><td>{u['email']}</td><td>{u['created_at']}</td></tr>"
            for u in users
        )
...
def analyze_number():
    try:
        ...
        num_data = requests.get(
            f"http://apilayer.net/api/validate?access_key={NUMVERIFY_KEY}&number={number}", timeout=8
        ).json()
        ipqs_data = requests.get(
            f"https://ipqualityscore.com/api/json/phone/{IPQS_KEY}/{number}", timeout=8
        ).json()
        ...
        return jsonify({
            "number": number, "valid": valid, "country": country, "carrier": carrier,
...
def vt_proxy(path):
    ...
    path_map = {'url': 'urls', 'ip': 'ip_addresses', 'domain': 'domains'}
    parts = path.split('/')
